...
class SpotifyEmbedFormatter extends FormatterBase {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    // Check if this is a media entity with a Spotify source.
    if ($media_entity && $media_entity->getEntityTypeId() === 'media') {
      $media_source = $media_entity->getSource();

      if ($media_source instanceof Spotify) {
        foreach ($items as $delta => $item) {
          $uri = $media_source->getMetadata($media_entity, 'uri');
          $type = $media_source->getMetadata($media_entity, 'type');
